# 更新记录 
* pg.20240809-1503.zip 【对不起，我是阿里不限速：阿里,UC,115,夸克使用本zip包不限速，阿里原画不需要svip也不用三方权益包，播放阿里内容不需要115账号秒传也不需要115VIP会员，115的分享内容无需付费即可欣赏，115分享大包和阿里分享大包不用建索引全部秒搜秒播，UC原画不需要会员，夸克原画非会员3G以内的视频随意看，88VIP20G以内的视频随便看。所有网盘全自动删除转存垃圾文件，不限次数。注意：不支持仅使用jar，必须zip完整解压使用。网盘原画仅支持原版 影视、OK影视、EasyBox】

本日更新内容：
1.所有玩偶系csp支持自动探测可用域名和指定域名。更新玩偶表哥域名，具体参考jsm.json和UI。去掉重复的网址。
2.对阿里秒传115增加保护代码，防止115故意使用异常参数崩溃播放端。
* pg.20240808-2131.zip 【对不起，我是阿里不限速：阿里,UC,115,夸克使用本zip包不限速，阿里原画不需要svip也不用三方权益包，播放阿里内容不需要115账号秒传也不需要115VIP会员，115的分享内容无需付费即可欣赏，115分享大包和阿里分享大包不用建索引全部秒搜秒播，UC原画不需要会员，夸克原画非会员3G以内的视频随意看，88VIP20G以内的视频随便看。所有网盘全自动删除转存垃圾文件，不限次数。注意：不支持仅使用jar，必须zip完整解压使用。网盘原画仅支持原版 影视、OK影视、EasyBox】

本日更新内容：
1.修复阿里被误打开的opentoken依赖。
2.阿里云盤資源增加秒傳115支持，默認開啓，會自動把阿里資源秒傳到115並使用115地址播放，秒傳失敗則繼續使用阿里播放。該功能可以在網盤設置的阿里設置中關閉。定制aliproxy路徑的有緣人需要更新aliproxy.gz
3.優化阿里云盤自動秒傳邏輯，增加秒傳成功提示。
4.取消阿里秒傳115的最大5G限制，如果是115非VIP则秒傳5G以上的視頻會失敗（视频依然不限速播放，只是不用115的cdn）
5.修復夸克和UC分享無法處理分享密碼的問題。
6.优化迅雷和PikPak转存播放。
7.阿里和夸克默认线程数（没有tokenm配置的情况下）改为32.
