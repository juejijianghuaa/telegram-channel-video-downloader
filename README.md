# 更新记录 
* pg.20240802-0154.zip 115分享尝试兼容tvbox（影视仓等），把顶级分类增加标识typeFlag:1。在所有网盘分享的列表中点击文件时，如果文件大于10G则在播放时会直接定位到该文件。对所有网盘分享类csp增加列表缓存，防止反复请求api，极大提高二次加载速度。修复115分享中的iso文件无法外部打开的问题（因为115升级了风控）
