...
4.因现在zip加载附加程序较多，对内存有一定压力，因此在网盘配置》订阅设置中增加了是否初始化WGCF的功能（默认不初始化）以及网盘设置》网盘相关设置-》tgsearch是否初始化（默认初始化）。tgsearch除了受此开关影响还受到是否配置了session以及tgearchapi地址是否本机来判断，如果都不满足则不会初始化本机tgsearch来节省内存。其实我主要是要节约内存看蓝光
