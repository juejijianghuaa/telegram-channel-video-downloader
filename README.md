# 更新记录 
* pg.20240806-1025.zip 【今日阿里,UC,115,夸克使用本zip包不限速，阿里原画不需要svip也不用三方权益包，不需要115账号秒传也不需要115会员，115的分享内容无需付费即可欣赏，UC原画不需要会员，夸克原画非会员3G以内的视频随意看，88VIP20G以内的视频随便看。注意：不支持仅使用jar，必须zip完整解压使用】
1. 优化csp_Wogg的域名选择机制，默认自动探测，也可以手工指定，可以指定多个网址用"|"分割。默认情况下无需任何设置，会在首次打开wogg时自动检查可用域名并使用之。
