# 更新记录 
* pg.20240620-1349.zip 修复csp_Youtube在安卓6及以下无法使用问题，是安卓6系统BUG引起，因此在安卓6上将会自动忽略指定代理，需要用全局代理或路由器代理使用Youtube。因为迅雷第三方加速全面完蛋以及PP需要会员才能愉快加速磁力，所以迅雷相关的so不再包含于jar里，改在zip的lib目录下（也会自动加载），下一版在lib中也不再包含迅雷相关so，如果需要迅雷加速so的有缘人可以把这版保存一下，新版更新时把本版的lib中的几个so复制进去即可。
