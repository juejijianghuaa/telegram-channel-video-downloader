...
本日更新内容：
1.修復aliproxy在低端設備上加速大文件時的卡頓問題。因爲golang的實現問題，原來默認使用的32bit aliproxy對于加速超過4G的文件表現不佳，因此默認改爲使用64bit版本的aliproxy。提示：只能安裝32位apk的盒子或電視通常本身也是64位cpu，因此正常運行64位aliproxy應該沒有問題，如果遇到無法正常運行的情況，請把tokenm.json中的"aliproxy_url":"./aliproxy.gz"改爲"aliproxy_url":"./aliproxy.32.gz"
