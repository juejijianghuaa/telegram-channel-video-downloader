# 更新记录 
* pg.20240808-1604.zip 【对不起，我是阿里不限速：阿里,UC,115,夸克使用本zip包不限速，阿里原画不需要svip也不用三方权益包，不需要115账号秒传也不需要115VIP会员，115的分享内容无需付费即可欣赏，115分享大包和阿里分享大包不用建索引全部秒搜秒播，UC原画不需要会员，夸克原画非会员3G以内的视频随意看，88VIP20G以内的视频随便看。所有网盘全自动删除转存垃圾文件，不限次数。注意：不支持仅使用jar，必须zip完整解压使用】
