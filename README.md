# 更新记录 
* pg.20240805-1530.zip 【今日阿里,UC,115,夸克使用本jar不限速，阿里原画不需要svip也不用三方权益包，不需要115账号秒传也不需要115会员，115的分享内容无需付费即可欣赏，UC原画不需要会员，夸克原画非会员3G以内的视频随意看，88VIP20G以内的视频随便看】
所有分享csp列表页增加文件夹标识。csp_Wogg支持自定义域名，在jsm.json中的wogg.json后增加csp标识后可以打开自定义域名功能，会在该csp的列表页看到“配置域名”分类。修复字幕推送功能，方法：http://播放器ip:9978/proxy?do=push, 因所连接的字幕站被墙，因此自动调用系统的"proxy“代理标识来连接字幕站，该功能的修复是为了115分享的大量REMUX和ISO没有中文字幕，需要用推送字幕的方式完美欣赏。
