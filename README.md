# 更新记录 
* pg.20240802-1406.zip 今日阿里使用本jar不限速（以後這句話會每次更新都加直到阿里關門，防止有緣人被其他群誤導）。115支持使用aliproxy加速，設置綫程為2即可，再往大了調我也沒測，理論上最高可能是4，使用大于1的好處是網絡部分不用java來代理，似乎速度能快一些。增加iso配置是否複製到剪貼板然後用外部播放器打開之功能，默認用外部打開，沒有弄成直接跳播放地址是因爲我發現直接跳地址就跑瀏覽器去瘋狂下載了，每秒50MB我的天。所以這個iso外部播放功能其實也可以實現外部下載哈。修復115自己的盤無法播放問題。
